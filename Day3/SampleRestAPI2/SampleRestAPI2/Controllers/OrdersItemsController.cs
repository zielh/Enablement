...
namespace SampleRestAPI2.Controllers
{
    ...
    public class OrdersItemsController : ControllerBase
    {
        ...
        {
            IEnumerable<OrdersItemsDTO> data = await _unitOfWork.OrdersItems.GetAll()
                .Select(c => new OrdersItemsWithProductDTO
                {
                    Id = c.Id,
        ...
        {
            OrdersItems data = await _unitOfWork.OrdersItems.GetBySingle(x => x.Id == id);
